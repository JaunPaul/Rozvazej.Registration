<script lang="ts">
	...
	import { getRequiredIds, getVisibleIds, validateStepAsync } from './lib/utils/validation';
	...
	abstract class PageHelper {
		...
		static next = async (step: 'step2' | 'step3') => {
			...
			const { ok, fieldErrors } = await validateStepAsync(currentStep, valuesToValidate);
